    ...
    struct ConstantText
    {
        ...
        void init(wxFont init_font)
        {
            ...
            version = _L("Version") + " " + std::string(SLIC3R_VERSION) + "+BOSS" +
                (std::string(SLIC3R_GIT_HASH).empty() ? "" : " (" + std::string(SLIC3R_GIT_HASH) + ")"
                );
            ...
            if (this->plater_ != nullptr && (m_app_updater->get_triggered_by_user() || app_config->get("notify_release") == "all")) {
                ...
                if (*Semver::parse(SLIC3R_VERSION) < *Semver::parse(evt_string)) {
                    ...
                    this->plater_->get_notification_manager()->push_version_notification( notif_type
                        ...
                        , [](wxEvtHandler* evnthndlr) {wxGetApp().open_browser_with_warning_dialog("https://github.com/mjonuschat/PrusaSlicer/releases"); return true; }
                    );
                }
            }
            });
